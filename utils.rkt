...
; Adapted from Thomas Gilray assignment 5 utils
...
(define prims-list '(= > < <= >= + - * / 
                     ...
                     hash hash-ref hash-set hash-count hash-keys hash-has-key? hash? hash-remove
...
(define (eval-llvm llvm-str)
  ...
  (when (not recent-header)
        ...
        (system (string-append clang++-path " -std=c++11 -pthread header.cpp -S -emit-llvm -o header.ll")))
  (define header-str (read-string 999999 (open-input-file "header.ll" #:mode 'text)))
